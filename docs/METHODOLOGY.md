...
| 指标 | 公式 | 含义 |
|------|------|------|
| **HVI**（主指标） | `wrong ÷ (correct + wrong)` | 引注幻觉率，**越低越好**。wrong = ✗MA+✗T |
...
> **判定范围说明**：本引擎为**确定性引注核验**，只核验「法条是否存在 / 是否为编造」与「是否援引已废止或旧版本法条（时态幻觉）」。它**不评判**引注法条的**实质性内容是否忠实**（Not Faithful, ✗NF）或**孤立事实是否正确**（✗F）——后者需要内容级语义比对或 LLM 裁判，与本项目「可复现、不依赖主观裁判」的设计原则冲突，故当前**不产出** ✗NF / ✗F 两类状态。文档与看板中的状态体系已与此保持一致；如未来引入内容级裁判，再扩展该集合。
...
- **逐题诊断矩阵**展示 `N` 次取样的**多数判定**（majority verdict）；若 `N` 次无严格
  多数，按 `✗MA > ✗T > ✗ERR > ✓ > · > ?` 的保守优先级取代表判定（对「幻觉监测」
