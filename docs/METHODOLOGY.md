...
1. **题目**：从题库抽取「可验证引注」类问题（当前 30 题，双语 `prompt_en`），每题对应一个预期法条（如《民法典》第 584 条）。
...
- **HVI / CRFI / Coverage / Integrity / Temporal** 改为跨**全部**取样汇总：
  ...
  即把每题每模型的 1 个样本点，摊薄成 `题目数 × N` 个样本点（当前 30 题 × 3 模型 × 3
  取样 = 270 个样本点），方差随样本量增大而显著收敛，HVI 跨轮稳定。

- **引注数（KPI）** = 多数判定为「含引注」（✓ 或 ✗）的题数，保持 0–30 的直观口径。
