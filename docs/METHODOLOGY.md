...
| 指标 | 公式 | 含义 |
|------|------|------|
| **HVI**（主指标） | `wrong ÷ (correct + wrong)` | 引注幻觉率，**越低越好**。wrong = ✗MA+✗T+✗NF |
...
| `✗MA` | 错引真实法条：援引了真实存在但错误的法条（与预期不符）| 计入 HVI/CRFI/Integrity 的 wrong |
| `✗NF` | NOT_FOUND：引注法条不存在（条号超出该法已知有效范围，即编造了不存在的法条）| 计入 HVI/CRFI/Integrity 的 wrong（与 ✗MA 同为幻觉，仅细化分类）|
...
> **判定范围说明**：本引擎为**确定性引注核验**，核验「法条是否真实存在」「是否编造不存在的法条（NOT_FOUND, ✗NF）」「是否为错引的真实法条（✗MA）」与「是否援引已废止或旧版本法条（时态幻觉, ✗T）」。其中 **✗NF** 通过 `config/statute_equivalence.json` 的 `article_ranges`（每法有效条号范围）判定：现行法引注的条号超出该法已知有效范围即视为法条不存在。它**不评判**引注法条的**实质性内容是否忠实**（Not Faithful, ✗F）或**孤立事实是否正确**——后者需要内容级语义比对或 LLM 裁判，与本项目「可复现、不依赖主观裁判」的设计原则冲突，故当前**不产出** ✗F 状态。文档与看板中的状态体系已与此保持一致；如未来引入内容级裁判，再扩展该集合。
...
- **逐题诊断矩阵**展示 `N` 次取样的**多数判定**（majority verdict）；若 `N` 次无严格
  多数，按 `✗MA > ✗NF > ✗T > ✗ERR > ✓ > · > ?` 的保守优先级取代表判定（对「幻觉监测」
