...
| `✗F` | 内容不忠实（Faithfulness fail）：引注条号真实且未废止，但对该法条内容的表述与官方条文实质不符（概括失真 / 要件遗漏 / 错配）。**可选增强层，默认关闭**（`--check-faithfulness`）。| **不计入** HVI/CRFI（保持主榜单不变）；仅作为独立副指标 `Content Fidelity` 报告；看板诊断矩阵以橙色标注 |
...
- **逐题诊断矩阵**展示 `N` 次取样的**多数判定**（majority verdict）；若 `N` 次无严格
  多数，按 `✗MA > ✗NF > ✗T > ✗ERR > ✗F > ✓ > · > ?` 的保守优先级取代表判定（对「幻觉监测」
