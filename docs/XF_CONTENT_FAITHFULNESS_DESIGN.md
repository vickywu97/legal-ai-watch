...
- `config/article_texts.json`（**已扩至 61 条，_uncovered=0**）：键 `law#article` → 官方条文**全文**。其中 38 条逐条取自 legal-hallucination-bench 的 `knowledge_base/laws/statutes.jsonl`（2327 条，verification_status 全 `verified`，源自全国人大/中国政府网官方文本）；**另有 23 条范围外规范的官方全文由用户提供的官方 .doc 原文（flk.npc.gov.cn / www.gov.cn 下载件）逐条抽取填入并标记 VERIFIED**（个人所得税专项附加扣除暂行办法 #5–#22、个人信息保护法 #38/#54、反不正当竞争法 #6/#7、数据安全法 #27）。**全部不凭记忆撰写**；39 题的全部引注均已纳入 ✗F 覆盖，无遗漏。反不正当竞争法为 **2025 修订版**，其「混淆行为」位于第 7 条（Q27 引注已同步改 #7），第 6 条为「社会监督」保留备用。
  ...
  - **范围外规范（23 条缺口）人工核验工作流（已完成）**：38 条之外，39 题还引用了 23 个 8 部法之外的法条（个人所得税专项附加扣除暂行办法 #5–#22、个人信息保护法 #38/#54、反不正当竞争法 #6/#7、数据安全法 #27），这些**不在 LHB KB 内**，须律师从官方原文逐条核对填入。工作流：`--emit-pending config/article_texts_unverified.json` 生成留空模板（幂等，带 `cited_by`/`official_source`/`status`）；用户于 2026-09-03 提供 4 份官方 .doc 下载件，22 条原缺口已逐条抽取填入并标记 `VERIFIED`，经 `--merge-pending` 合并进主库（仅接受 VERIFIED 且非空者，主库 `_uncovered` 归零，合并条目在普通 regen 时被 `old_texts` 保留）。反不正当竞争法为 **2025 修订版**，其「混淆行为」移至第 7 条、第 6 条为「社会监督」——Q27 引注已同步由 #6 改为 #7，主库补 #7 原文，基准题目层与 ✗F 文本层现对齐（详见 `docs/ARTICLE_TEXTS_PENDING.md`）。
...
2. **正文扩写（已完成）**：`article_texts.json` 已从 10 条种子扩到 **61 条官方全文**（38 条来自已核验 KB + 23 条来自用户提供的官方 .doc，含反不正当竞争法 #6/#7），覆盖 39 题**全部**预期引注（`_uncovered=0`），无凭记忆撰写。
3. **粒度升级**（可选）：本地 embedding 替代字符二元文法，提升语义捕获能力（引入本地模型依赖）。
4. **测试（已绿）**：`pytest -q` → 62 passed（含 `TestCalibrationProbe`），无 error。
5. **范围外规范覆盖（已完成）**：23 条 8 部法之外缺口（原 22 条 + 反不正当竞争法 #7）已由用户提供的官方 .doc 原文逐条抽取填入并 `VERIFIED`，经 `--merge-pending` 合并进主库，`_uncovered` 归零。反不正当竞争法为 **2025 修订版**，其「混淆行为」位于第 7 条、第 6 条为「社会监督」——Q27 引注已同步改 #7，主库补 #7 原文，基准题目层与 ✗F 文本层对齐。模板与工作流保留供后续扩展。
