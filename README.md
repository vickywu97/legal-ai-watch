...
题库当前 **88 题**（由初期 31 题经多轮扩充：2026-09-01 增至 39、2026-09-03 并入 Q40–Q64 至 64、2026-09-05 并入 Q65–Q88 至 88），题型覆盖**跨法张冠李戴**（如公司/专利问题被错引至民法典）、**旧法时序陷阱**（诱导模型援引已废止的《合同法》《民法通则》→ 预期 `✗T`）、**硬幻觉（不存在条号）**、**超范围法律**（援引 8 部法之外的法律作答）、**修订陷阱**（如 2023 修订公司法 #27 由「一人公司」改为「决议不成立」、税收征管法滞纳金比率万分之五/日）。各题预期引注均落在 `config/article_texts.json` 已核验条文全文（**104 条、`_uncovered=0`**），并经 `scripts/verifier.py` 双路径验证（正确回答→`✓`、陷阱回答→`✗MA`/`✗T`）。
