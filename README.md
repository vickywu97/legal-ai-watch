...
本仓库的**实际评测与看板生成完全不依赖 submodule**：题库来自本仓库自带的 [`config/questions.json`](config/questions.json)，核验引擎 [`scripts/verifier.py`](scripts/verifier.py) 为本仓库自带的官方方法学实现。`legal-hallucination-bench` 是**私有仓库（需授权访问）**，仅作为**可选的题库上游源**被 [`scripts/sync_questions.py`](scripts/sync_questions.py) 引用（用于把上游最新题库同步进本仓库）；未拉取时（如离线）演示与评测照常可跑。

> 📦 **作品集联动**：本仓库是 [vickywu97 AI 法律作品集](https://github.com/vickywu97/vickywu97) 的「公开榜单」环节；地基评测 [legal-hallucination-bench](https://github.com/vickywu97/legal-hallucination-bench) 与产品 [compliance-triangle](https://github.com/vickywu97/compliance-triangle) 同属该作品集。
...
题库当前 **112 题**（由初期 31 题经多轮扩充：增至 39 → 并入 Q40–Q64 至 64 → 并入 Q65–Q88 至 88 → 并入 Q89–Q112 至 112），题型覆盖**跨法张冠李戴**（如公司/专利问题被错引至民法典）、**旧法时序陷阱**（诱导模型援引已废止的《合同法》《民法通则》→ 预期 `✗T`）、**硬幻觉（不存在条号）**、**超范围法律**（援引 8 部法之外的法律作答）、**修订陷阱**（如 2023 修订公司法 #27 由「一人公司」改为「决议不成立」、税收征管法滞纳金比率万分之五/日）。各题预期引注均落在 `config/article_texts.json` 已核验条文全文（**123 条、`_uncovered=0`**），并经 `scripts/verifier.py` 双路径验证（正确回答→`✓`、陷阱回答→`✗MA`/`✗T`）。
